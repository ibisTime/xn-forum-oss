$(function() {
    ...
    var hasParentCode = !!getQueryString('p');
    ...
    var fields = [{
        ...
        title: '父菜单',
        type: 'select',
        url: code ? $('#basePath').val() + '/view/list/company?type=1&companyCode=' + (getCityId(getUserId) || 0) : $('#basePath').val() + '/view/list?type=1&companyCode=' + (getCityId(getUserId) || 0),
        keyName: 'code',
        valueName: 'name',
        ...
        required: true,
        type: 'select',
        hidden: code && !hasParentCode,
        data: {
            '1': '内部',
            '2': '外部'
        },
        onChange: function(r) {
            if (hasParentCode || !code) {
                if (r == 1) {
                    $('#url1').parent().show();
                    $('#url').parent().hide();
